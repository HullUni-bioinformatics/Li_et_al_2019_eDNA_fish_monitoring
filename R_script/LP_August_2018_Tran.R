...
NMDS_12S_FIG <- ggplot() + 
  ...
  geom_point(data=NMDS_12S_data.scores,aes(x=NMDS1,y=NMDS2,colour=Lake),size=3.5) + # add the point markers
...
ggsave("FigS1_corr.jpeg",corr_fig2,path = "Lake project/Figures_new/",  width = 10, height = 10, units = "in", dpi=500)
...
DAFOR_Ind<- ggplot(mere_excl_absent, aes(x = Ind_abu, y = DAFOR_c))+
            ...
            scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                          labels = trans_format("log10", math_format(10^.x)))+
            geom_smooth(formula= y ~ x,method="lm", se=FALSE,lwd=0.5,na.rm=TRUE, colour='black')+
            stat_cor(method = "spearman",label.sep = "\n",label.y=1,label.x=2.8,size=3.5,hjust = 0)+
            labs(x="Individual density (ind./ha)", y="DAFOR scale based on eDNA data")+theme_bw()+
            theme(text=element_text(size=12),strip.background = element_blank(),strip.text=element_text(face="bold"),
                  panel.grid.minor=element_blank(),panel.grid.major =element_blank())+
            geom_text_repel(aes(label=Species),size=3,fontface ="italic")
...
#FigS3_Bio####
ggsave("FigS3_Bio.jpeg",DAFOR_3, path = "Lake project/Figures_new/",  width = 6.5, height = 4, units = "in", dpi=500)
...
ggsave("Fig3_DAFOR.pdf",DAFOR_FNL, path = "Lake project/Figures_new/",  width = 8, height = 9, units = "in", dpi=500)
...
###FigS2_Fish_com_reads####


ggsave("FigS2_Fish_com_reads.jpeg",path = "Lake project/Figures_new/",  width = 15, height = 8, units = "in", dpi=500)
...
Clu_Cytb <- fviz_dend(res.hc_cytb, k = 4, # Cut in three groups
                      ...
                      k_colors = c("#669bb7", "#638d48", "#5541a7","#c34837"),
...
Clu_sRNA <- fviz_dend(res.hc_sRNA, k = 4, # Cut in three groups
                      cex = 1,lwd = 1, xlab = "Sampling lake", main = "", ggtheme = theme_bw(base_size = 20),# label size
                      k_colors = c("#638d48", "#c34837","#669bb7", "#5541a7"),
                      color_labels_by_k = TRUE, # color labels by groups
                      rect = TRUE # Add rectangle around groups
                      )+theme(panel.grid.minor=element_blank(),panel.grid.major =element_blank())
...
NMDS_Clu_fig1 <- grid.arrange(Clu_Cytb,Clu_sRNA,NMDS_Cytb_FIG,NMDS_12S_FIG, ncol = 2, nrow = 2,
...
ggsave("Fig5_RFA_SL.jpeg", path = "Lake project/Figures_new/",  width = 8, height = 6, units = "in", dpi=500)
...
####FigS4_Tran####
ggsave("FigS4_Tran.jpeg",TRAN_fig2, path = "Lake project/Figures_new/",  width = 11, height = 6.5, units = "in", dpi=500)
...
ggsave("FigS5_Sp.jpeg",Tran_Sp, path = "Lake project/Figures_new/",  width = 9, height = 11, units = "in", dpi=500)
